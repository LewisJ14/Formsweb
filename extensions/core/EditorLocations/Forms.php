...
namespace IPS\form\extensions\core\EditorLocations;
...
class _Forms
{
	...
	public function attachmentPermissionCheck( $member, $id1, $id2, $id3 )
	{
		try
		{
			$form = \IPS\form\Form::load( $id1 );
			return $form->can( 'view', $member );
		}
	...
	protected function performRebuild( $offset, $max, $callback )
	{
		...
		/* Language bits */
		foreach( \IPS\Db::i()->select( '*', 'core_sys_lang_words', "word_key LIKE 'form_form_%_desc' OR word_key LIKE 'form_form_%_rules' OR word_key LIKE 'form_form_%_log' OR word_key LIKE 'form_form_%_pm_message' OR word_key LIKE 'form_form_%_email_message' OR word_key LIKE 'form_form_%_topic_post'" ) as $word )
		{
			try
			{
	...
	{
		return \IPS\Db::i()->select( 'COUNT(*)', 'core_sys_lang_words', "word_key LIKE 'form_form_%_desc' OR word_key LIKE 'form_form_%_rules' OR word_key LIKE 'form_form_%_log' OR word_key LIKE 'form_form_%_pm_message' OR word_key LIKE 'form_form_%_email_message' OR word_key LIKE 'form_form_%_topic_post'" )->first();
	}
}